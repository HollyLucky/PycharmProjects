...
def inference(images,batch_size,n_classes):
    ...
    with tf.variable_scope('pooling2_lrn') as scope:
        norm2=tf.nn.lrn(conv2,depth_radius=4,bias=1.0,alpha=0.001/9.0,name='norm2')
    ...
    with tf.variable_scope('local4')as scope:
        ...
        biases=tf.get_variable('biases',shape=[128],dtype=tf.float32,initializer=tf.constant_initializer(0.1))
        local4=tf.nn.relu(tf.matual(local3,weights)+biases,name='local4')
...
def losses(logits,labels):
    with tf.variable_scope('loss')as scope:
        cross_entrpy=tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,labels=labels,name='xentropy_per_example')
        loss=tf.reduce_mean(cross_entrpy,name='loss')

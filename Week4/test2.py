...
b_fc1=weight_variable([1024])#一共1024个节点
#池化层2的输出扁平化为1维
h_pool2_flat=tf.reshape(h_pool2,[-1,7*7*64])
#求一个第一全连接层的输出
h_fc1=tf.nn.relu(tf.matmul(h_pool2_flat,W_fcl)+b_fc1)
#keep_probe用来表示神经元的输出概率
keep_probe=tf.placeholder(tf.float32)
h_fcl_drop=tf.nn.dropout(h_fc1,keep_probe)
#初始化第二个全连接层
...
with tf.Session() as sess:
    ...
    for e in range(21):
        for batch in range(n_batch):
            ...
            sess.run(train_step,feed_dict={x:batch_xs,y:batch_ys,keep_probe:0.7})
        acc=sess.run(accuracy,feed_dict={x:mnist.test.images,y:mnist.test.labels,keep_probe:1.0})
